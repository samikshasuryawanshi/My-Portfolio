...
const Projects = () => {
  ...
  return (
    <div id='projects' className='min-h-screen overflow-hidden projectSection text-white p-10'>
        <h1 className='lg:text-3xl mt-10  gradient-border-Project'>My Portfolio Highlight's</h1>
         <div className='lg:flex lg:flex-wrap gap-11 lg:mt-20 mt-10'>
            ...
            {ProjectsData.map((project, index) => (
              <div ref={addToProjectRefs} key={index} className='projects cursor-pointer lg:p-4 p-2 lg:w-[31%] w-full lg:mt-0 mt-5  h-fit rounded-lg shadow-lg flex flex-col items-center shrink-0'>
               <div className='relative w-full h-60 overflow-hidden rounded-lg mb-4 group'>
                  {/* Image */}
                  <img src={project.image} alt={project.title} className='w-full h-full object-cover rounded-lg group-hover:brightness-75 transition duration-300' />

                  {/* Play Button */}
                  <div className='absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300'>
                    <button
                      onClick={() => window.open(project.videoLink, '_blank')}
                      className='bg-white text-black px-3 py-2 rounded-full shadow-lg hover:scale-110 transition-transform duration-300'
                    >
